...
function CertificationCard({ item, setselected, reload , report }) {
    ...
    const sendData = async (url , status) => {
        ...
        let res = await axios.patch(url , { "block" : status});
        return res.data;
    };
        ...
        <>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', padding: '40px', paddingTop: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: item.certify == 'Blocked' ? '#FF7F7F' : '#2575c0', marginTop: '50px', marginLeft: '10px', width: '45%', height: '80%', minHeight: '350px', borderRadius: '15px' }}>
                   ...
                   { report == 'true' ? null : <button style={{ height: '36px' , width : '220px', fontSize: '14px', fontFamily: 'Roboto, sans-serif', color: '#ffffff', backgroundColor: item.block == 'Certified' ? '#FF7F7F' : "#2575c0", border: '2px solid white', borderRadius: '8px' }} type="button" onClick={() => {

                        if (item.certify == 'Certfied')
                        {
                            block('');
                        }
                        else{
                    ...
                    }}> <img style={{width : '20px' , height : '20px'}} src="https://img.icons8.com/external-kiranshastry-solid-kiranshastry/64/ffffff/external-delete-miscellaneous-kiranshastry-solid-kiranshastry.png"/> <p style = {{display : 'inline'}}> {item.block == 'Certified' ? 'Remove certification' : 'Certify Account'} </p> </button>
}
